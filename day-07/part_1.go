...
func (a *amplifier) process() bool {
	switch opCode := a.seq[a.ptr]; opCode {
	case 99:
		return true
	default:
		a.handleComplexOpCode()
	}
	return false
}
...
func (a *amplifier) handleComplexOpCode() {
...
func (a *amplifier) run() int {
	for {
		if finished := a.process(); finished {
			break
		}
	}
	return a.output
...
func main() {
	...
	for _, pss := range permutations([]int{0, 1, 2, 3, 4}) {
		var prevOutput int
		for _, ps := range pss {
			inputs := []int{ps, prevOutput}
			a := newAmplifier(inputs, seq)
			prevOutput = a.run()
		}
		thrusterSignals = append(thrusterSignals, prevOutput)
	}
